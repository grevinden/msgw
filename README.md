...
| `503` | Upstream не прошёл health-чек (TCP-проверка порта). MSGW проверяет, что порт upstream открыт, но не отправляет HTTP-запросы. Первый запрос к новому хосту включает проверку (до 2 сек). Последующие запросы используют результат фонового чекера (каждые 3 сек) |
...
> **Примечание:** Параметры Uvicorn (`--host`, `--port`, `--workers` и др.) задаются через CLI аргументы в `CMD` Dockerfile, а не через переменные окружения. Изменить их можно, переопределив `CMD` при запуске контейнера. Полный список параметров — в [Dockerfile](Dockerfile).
